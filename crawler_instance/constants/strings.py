
class GENERIC_STRINGS:
    S_EMPTY = ""
    S_SPACE = " "

class INFO_MESSAGES:
    S_LOADING_BACKUP_URL = "[1] Loading Backup URL"
    S_BACKUP_QUEUE_EMPTY = "[2] Backup Queue Empty"
    S_THREAD_CREATED = "[3] THREAD CREATED : "

class ERROR_MESSAGES:
    S_SINGLETON_EXCEPTION = "[1] This class is a singleton"
    S_DATABASE_FETCH_ERROR = "[2] Database Load Error : Database Empty"
    S_URL_PROCESSING_ERROR = "[3] ERROR PROCESSING"
    S_FIILE_PARSED = "[4] Successfully Parsed File"
    S_URL_PARSED = "[5] Successfully Parsed URL"
    S_BACKUP_PARSED = "[6] Successfully Saved Backup URL"