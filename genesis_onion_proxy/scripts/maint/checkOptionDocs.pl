...
open(F, "/cygdrive/c/temp/tor-0.4.6.6/src/app/tor --list-torrc-options |") or die;
while (<F>) {
...
loadTorrc("/cygdrive/c/temp/tor-0.4.6.6/src/config/torrc.sample.in", \%torrcSampleOptions);

# Try to figure out what's in the man page.

my $considerNextLine = 0;
open(F, "/cygdrive/c/temp/tor-0.4.6.6/doc/tor.1.txt") or die;
while (<F>) {
